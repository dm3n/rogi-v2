...
import { RenewalDetails } from '@/lib/types'
...
interface Props {
  data: RenewalDetails
  onChange: (data: RenewalDetails) => void
  onNext: () => void
  onBack: () => void
}

export default function Step3Renewal({ data, onChange, onNext, onBack }: Props) {
  const set = <K extends keyof RenewalDetails>(k: K, v: RenewalDetails[K]) => onChange({ ...data, [k]: v })

  return (
    <div className="max-w-md mx-auto">
      <h1 className="text-2xl font-semibold text-gray-900 mb-1">Renewal details</h1>
      <p className="text-gray-400 text-sm mb-8">All fields are optional.</p>

      <div className="space-y-4">
        <CurrencyInput label="Current Mortgage Balance" value={data.currentMortgageSize} onChange={(v) => set('currentMortgageSize', v)} optional />
