...
import { PreApprovalDetails } from '@/lib/types'
...
interface Props {
  data: PreApprovalDetails
  onChange: (data: PreApprovalDetails) => void
  onNext: () => void
  onBack: () => void
}

export default function Step3PreApproval({ data, onChange, onNext, onBack }: Props) {
  const set = <K extends keyof PreApprovalDetails>(k: K, v: PreApprovalDetails[K]) => onChange({ ...data, [k]: v })

  return (
    <div className="max-w-md mx-auto">
      <h1 className="text-2xl font-semibold text-gray-900 mb-1">Pre-approval details</h1>
      <p className="text-gray-400 text-sm mb-8">All fields are optional.</p>

      <div className="space-y-4">
        <div className="space-y-2">
          <CurrencyInput
            label="Ideal Purchase Price"
            value={data.unknownPrice ? '' : data.idealPurchasePrice}
            onChange={(v) => set('idealPurchasePrice', v)}
            optional
          />
          <div className="flex items-center gap-2">
            <Checkbox
              id="unknownPrice"
              checked={data.unknownPrice}
              onCheckedChange={(checked) => set('unknownPrice', !!checked)}
            />
            <label htmlFor="unknownPrice" className="text-sm text-gray-500 cursor-pointer">
